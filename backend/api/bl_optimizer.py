...
def build_views(tickers, target_date=None):
    """
    For each ticker:
    - Fetch current price
    ...
    for symbol in tickers:
        try:
            ...
            if cached:
                ...
                forecast = TrendPredictor.forecast_price(symbol, allow_lstm=False, target_date=target_date)
...
def run_black_litterman(tickers, risk_tolerance=0.5, target_date=None):
    """
    Full BL pipeline:
    ...
    try:
        ...
        # --- LSTM views ---
        views, confidences, view_details = build_views(valid_tickers, target_date=target_date)
